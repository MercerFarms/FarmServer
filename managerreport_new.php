...
<?php
	if (isset($_REQUEST['startdate'], $_REQUEST['enddate'])) {
		...
		$sql = "SELECT ".$grouping.", sum(Diesel) as Diesel, max(UnitHours) as UnitHours, max(ServiceDue) as ServiceDue, (ServiceDue-UnitHours) as Total, DEF, YEAR(Timestamp) AS YearFrame, ".$timefn."(Timestamp) AS TimeGroup, ".$timefn."('".$startdate."') AS StartFrame, ".$timefn."('".$enddate."') AS EndFrame ";
		$sql .= "FROM FuelService ";
		$sql .= "WHERE Timestamp >= '".$startdate."' AND Timestamp <= '".$enddate."30' ";
		$sql .= "GROUP BY ".$grouping.", YEAR(Timestamp), ".$timefn."(Timestamp) ";
		$sql .= "order by ".$grouping.", YEAR(Timestamp) asc, ".$timefn."(Timestamp) asc;";
		echo "<p>".$sql."</p>";
		...
		echo "<tr><th></th><th></th>";
		for ($i=0; $i<sizeof($shows); $i++) {
			$dres = mysql_query("select min(StartMeter) as beginmeter, max(EndMeter) as finalmeter, sum(EndMeter-StartMeter) as meter from TruckFueling where ".$timefn."(Day)=".$timefn."('".$shows[$i]."') group by ".$timefn."(Day);") or die(mysql_error());
			$drow = mysql_fetch_array($dres);
			echo "<th colspan='3'>meter</th><th>".$drow['beginmeter']."</th><th>".$drow['finalmeter']."</th><th>".$drow['meter']."</th>";
		}
		echo "</tr>";
		$currframe = $startframe;
